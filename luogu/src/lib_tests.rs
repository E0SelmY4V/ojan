...
fn big_integer() {
    macro_rules! tcmp {
        ...
        ($a:literal, $op:ident, $b:literal, $t:ty) => {
            assert!(&<BigInteger<$t>>::from($a as i128).$op(&<BigInteger<$t>>::from($b as i128)));
        };
    }
    tcmp!(0x100001 > 0x00);
    tcmp!(0x100001 < 0x101101);
    ...
    macro_rules! tfmt {
        ...
        ($num:literal, $t:ty) => {
            assert_eq!(
                stringify!($num),
                format!("{}", <BigInteger<$t>>::from($num as i128))
            );
        };
    }
    ...
    macro_rules! tcalc {
        ...
        ($u1:literal, $u2:literal, $ops:ident, $t:ty) => {
            let t = tcalc!($u1, $u2, $t);
            assert!(t.0.clone().$ops(t.1.clone()) == <BigInteger<$t>>::from(i128::$ops($u1, $u2)));
            assert_eq!(t, tcalc!($u1, $u2, $t));
        };
        ($u1:expr, $u2:expr, $t:ty) => {
            (
                <BigInteger<$t>>::from($u1 as i128),
                <BigInteger<$t>>::from($u2 as i128),
            )
        };
    }
    ...
    tcalc!(14 / 5);
    tcalc!(-1882727373 / -1992838991929);
    tcalc!(-29291993716637271_i128 / -998244353_i128);
    tcalc!(0 / 998244353_i128);
    tcalc!(0 / 99);
    ...
    macro_rules! tinput {
        ...
        ($l:literal, $t:ty) => {
            assert!(
                stringify!($l).parse::<BigInteger<$t>>().unwrap() == <BigInteger<$t>>::from($l)
            );
        };
    }
